...
class Multi_Encoder(nn.Module):
    def __init__(self, num_users, num_items, dim_each_feedback, n_layers, behavior_names, behavior_type, sparse_graph_flag, combine_choice = 'mean', acti_func = 'ReLU', graph_module = "LightGCN", \
    ...
    def forward(self, multi_adjs, init_user_embeddings = None, init_item_embeddings = None):
        ...
        # --------------- forward --------------- 
        if self.graph_module == "LightGCN":
            for i in range(len(self.encoders_list)):
                cur_encoder = self.encoders_list[i]
                #print(cur_encoder)
                #print('--------')
        ...
        if self.combine_choice == 'mean':
            ...
            if len(EP_index) > 0:
                ...
                EP_embeddings_user = F.relu(torch.matmul(torch.mean(user_embeddings_each_behavior[EP_index], dim = 0), self.weight_dic["fine_weight_EP_u"]))
                EP_embeddings_item = F.relu(torch.matmul(torch.mean(item_embeddings_each_behavior[EP_index], dim = 0), self.weight_dic["fine_weight_EP_v"]))
            ...
            if len(IP_index) > 0:
                ...
                IP_embeddings_user = F.relu(torch.matmul(torch.mean(user_embeddings_each_behavior[IP_index], dim = 0), self.weight_dic["fine_weight_IP_u"]))
                IP_embeddings_item = F.relu(torch.matmul(torch.mean(item_embeddings_each_behavior[IP_index], dim = 0), self.weight_dic["fine_weight_IP_v"]))
            ...
            if len(EN_index) > 0:
                ...
                # print("user_embeddings_each_behavior[EN_index]=",user_embeddings_each_behavior[EN_index])
                # if torch.isnan(user_embeddings_each_behavior[EN_index]).any():
                #     print("nan in user_EN before")
                #     input("pause")
                # if torch.isinf(user_embeddings_each_behavior[EN_index]).any():
                #     print("inf in user_EN before")
                #     input("pause")

                # print("item_embeddings_each_behavior[EN_index]=",item_embeddings_each_behavior[EN_index])
                # if torch.isnan(item_embeddings_each_behavior[EN_index]).any():
                #     print("nan in item_EN before")
                #     input("pause")
                # if torch.isinf(item_embeddings_each_behavior[EN_index]).any():
                #     print("inf in item_EN before")
                #     input("pause")
